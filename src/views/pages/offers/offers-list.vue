<template>
      ...
      <div class="page-header">
        <div class="add-item d-flex">
          <div class="page-title">
            <h4>Ofertas</h4>
            <h6>Administra las ofertas del sistema</h6>
          </div>
        </div>
        <ul class="table-top-head">
          <li>
            <a @click="loadOffers" data-bs-toggle="tooltip" data-bs-placement="top" title="Refrescar">
              ...
              <thead>
                <tr>
                  ...
                  <th @click="sortBy('nombre')" style="cursor: pointer;">
                    Oferta <i :class="getSortIcon('nombre')"></i>
                  </th>
              ...
              <tbody>
                ...
                <tr v-for="offer in sortedOffers" :key="offer.id">
                  ...
                  <td>
                    ...
                    <small class="text-muted" v-if="offer.tipo_aplicacion === 'multiple'">
                      <i class="ti ti-package"></i> {{ offer.items_count || 0 }} productos incluidos
                    </small>
                    <small class="text-muted" v-else>
                      Categoría: {{ offer.categoria_nombre || '-' }}<br>
                      Subcategoría: {{ offer.subcategoria_nombre || '-' }}
                    </small>
                  </td>
                  <td>
                    <span v-if="offer.tipo_descuento === 'porcentaje'">
                      {{ formatNumber(offer.valor_descuento) }}%
                    </span>
                    <span v-else>0.00</span>
                  </td>
                  <td>
                    <span v-if="offer.tipo_descuento === 'monto'">
                      {{ formatMoney(offer.valor_descuento) }}
                    </span>
                    <span v-else>{{ formatMoney(0) }}</span>
                  </td>
                  <td>{{ offer.agencia_nombre || 'AGENCIA PRINCIPAL' }}</td>
                  <td>
                    <span :class="getStatusClass(offer)">
                      {{ getStatusText(offer) }}
                    </span>
                  </td>
                  <td class="action-table-data justify-content-end">
                    <div class="edit-delete-action">
                      <a
                        class="me-2 p-2"
                        href="#"
                        @click.prevent="openViewModal(offer)"
                        data-bs-toggle="modal"
                        data-bs-target="#view-offer"
                      >
                      ...
                      <a
                        ...
                        data-bs-toggle="modal"
                        data-bs-target="#delete-offer-modal"
                      >
  ...
  <offer-modal
    :offer="selectedOffer"
    @offer-saved="onOfferSaved"
    @offer-deleted="onOfferDeleted"
  ></offer-modal>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Guardar Reporte - Ofertas</h5>
...
import OfferModal from '@/components/modal/offer-modal.vue';
...
export default {
  ...
  methods: {
    ...
    async openViewModal(offer) {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`http://localhost:3000/api/v1/offers/${offer.id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        this.selectedOffer = response.data.data;
        console.log('Offer loaded for view:', this.selectedOffer);
      } catch (error) {
    ...
    async openEditModal(offer) {
      try {
        ...
        console.log('Offer loaded for edit:', this.selectedOffer);
      } catch (error) {
        console.error('Error loading offer:', error);
        Swal.fire('Error', 'No se pudo cargar la oferta', 'error');
      }
    ...
    getStatusClass(offer) {
      ...
      return offer.is_active ? 'badge bg-success' : 'badge bg-danger';
    },
    ...
    getStatusText(offer) {
      ...
      return offer.is_active ? 'ACTIVO' : 'INACTIVO';
    },
    ...
    saveAsExcel() {
      ...
      try {
        ...
        const data = this.sortedOffers.map(offer => ({
          'Código': offer.codigo,
          'Oferta': offer.nombre,
          'Porcentaje': offer.tipo_descuento === 'porcentaje' ? `${this.formatNumber(offer.valor_descuento)}%` : '0.00',
          'Monto': offer.tipo_descuento === 'monto' ? parseFloat(offer.valor_descuento) : 0,
          'Agencia': offer.agencia_nombre || 'AGENCIA PRINCIPAL',
          'Estatus': this.getStatusText(offer)
        }));
    ...
    async saveAsPDF() {
      ...
      try {
        ...
        pdf.save(`ofertas_${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    async saveAsImage() {
      ...
      try {
        ...
        link.download = `ofertas_${new Date().toISOString().split('T')[0]}.png`;
    ...
    async buildReportHTML() {
      ...
      const tableRows = this.sortedOffers.map(offer => `
        <tr>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${offer.codigo}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${offer.nombre}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px; text-align: center;">
            ${offer.tipo_descuento === 'porcentaje' ? `${this.formatNumber(offer.valor_descuento)}%` : '0.00'}
          </td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px; text-align: right;">
            L ${offer.tipo_descuento === 'monto' ? this.formatMoney(offer.valor_descuento) : this.formatMoney(0)}
          </td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${offer.agencia_nombre || 'AGENCIA PRINCIPAL'}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px; text-align: center;">${this.getStatusText(offer)}</td>
        </tr>
      `).join('');
        ...
        <body>
          <div class="header-section">
            ...
            <div class="report-box">
              <div class="report-title">REPORTE DE OFERTAS</div>
