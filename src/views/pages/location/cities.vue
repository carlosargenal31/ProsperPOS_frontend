      ...
      <div class="page-header">
        <div class="add-item d-flex">
          <div class="page-title">
            <h4>Ciudades</h4>
            <h6>Administra las ciudades del sistema</h6>
          </div>
        </div>
        ...
        <div class="page-btn d-flex gap-2">
          <a
            ...
            @click="openAddModal"
          >
            <i class="ti ti-circle-plus me-1"></i>Agregar Nueva Ciudad
          </a>
                  ...
                  <td class="action-table-data justify-content-end">
                    <div class="edit-delete-action">
                      <a
                        class="me-2 p-2"
                        href="#"
                        @click.prevent="openViewModal(city)"
                        data-bs-toggle="modal"
                        data-bs-target="#view-city"
                        title="Ver detalles"
                      >
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Guardar Reporte - Ciudades</h5>
...
export default {
  ...
  computed: {
    sortedCities() {
      if (!this.cities || !Array.isArray(this.cities) || this.cities.length === 0) return [];
  ...
  methods: {
    ...
    saveAsExcel() {
      ...
      try {
        ...
        const data = this.sortedCities.map(city => ({
          'ID': city.id,
          'Nombre': city.nombre,
          'Departamento': city.state_name || '-',
          'Estado': city.is_active ? 'Activa' : 'Inactiva'
        }));
    ...
    async saveAsPDF() {
      ...
      try {
        ...
        pdf.save(`ciudades_${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    async saveAsImage() {
      ...
      try {
        ...
        link.download = `ciudades_${new Date().toISOString().split('T')[0]}.png`;
    ...
    async buildReportHTML() {
      ...
      const tableRows = this.sortedCities.map(city => `
        <tr>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${city.id}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${city.nombre}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${city.state_name || '-'}</td>
        ...
        <body>
          <div class="header-section">
            ...
            <div class="report-box">
              <div class="report-title">REPORTE DE CIUDADES</div>
