      ...
      <div class="page-header">
        ...
        <div class="page-btn d-flex gap-2">
          <a
            ...
            @click="openAddModal"
          >
            <i class="ti ti-circle-plus me-1"></i>Agregar Nuevo Departamento
          </a>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Guardar Reporte - Departamentos</h5>
...
export default {
  ...
  computed: {
    sortedStates() {
      if (!this.states || !Array.isArray(this.states) || this.states.length === 0) return [];
  ...
  methods: {
    ...
    saveAsExcel() {
      ...
      try {
        ...
        const data = this.sortedStates.map(state => ({
          'ID': state.id,
          'Nombre': state.nombre,
          'País': state.country_name || '-',
          'Estado': state.is_active ? 'Activo' : 'Inactivo'
        }));
    ...
    async saveAsPDF() {
      ...
      try {
        ...
        pdf.save(`departamentos_${new Date().toISOString().split('T')[0]}.pdf`);
    ...
    async saveAsImage() {
      ...
      try {
        ...
        link.download = `departamentos_${new Date().toISOString().split('T')[0]}.png`;
    ...
    async buildReportHTML() {
      ...
      const tableRows = this.sortedStates.map(state => `
        <tr>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${state.id}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${state.nombre}</td>
          <td style="padding: 6px; border: 1px solid #ddd; font-size: 9px;">${state.country_name || '-'}</td>
        ...
        <body>
          <div class="header-section">
            ...
            <div class="report-box">
              <div class="report-title">REPORTE DE DEPARTAMENTOS</div>
